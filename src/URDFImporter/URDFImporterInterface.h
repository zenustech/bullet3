...
public:
    ...
    //virtual std::vector<std::shared_ptr<zeno::PrimitiveObject>> getVisualShapes();
	...
	virtual int convertLinkVisualShapes(int linkIndex, const btTransform& inertialFrame) const { return -1; }
	...
	virtual class btCompoundShape* convertLinkCollisionShapes(int linkIndex, const btTransform& localInertiaFrame) const = 0;
